...
export default function PurchasedCourses() {
  ...
  const getUIText = (key: string) => {
    ...
    return texts[key as keyof typeof texts]?.[locale as keyof typeof texts[keyof typeof texts]] || 
           texts[key as keyof typeof texts]?.['en'] || key;
  };
    ...
    return (
      <div className="text-center p-12">
        <div className="bg-gray-50 rounded-xl p-12">
          ...
          <h3 className="text-xl font-semibold text-gray-800 mb-3">{getUIText('noCourses')}</h3>
    ...
    return (
      <div className="text-center p-12">
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-yellow-800 mb-2">{getUIText('dataError')}</h3>
          <p className="text-yellow-700 mb-4">{getUIText('dataErrorDesc').replace('{count}', courses.length.toString())}</p>
          <details className="text-left">
            <summary className="cursor-pointer text-yellow-800 font-medium">{getUIText('detailedInfo')}</summary>
                    ...
                    <div className="flex items-center space-x-6">
                      {course.setId!.duration && (
                        <div className="flex items-center text-gray-500">
                          ...
                          <span className="font-medium">{course.setId!.duration} {getUIText('minutes')}</span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center text-gray-500">
                        ...
                        <span className="font-medium">2 {locale === 'ka' ? 'დან' : locale === 'ru' ? 'из' : 'of'} 5 {getUIText('lessons')}</span>
                      </div>
                    </div>
                    ...
                    {course.setId!.thumbnailImage ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <div className="text-center">
                          ...
                          <span className="text-purple-500 text-sm font-medium">{getUIText('complex')}</span>
                        </div>
                      </div>
                    )}
                  </div>
                    ...
                    <div className='flex items-center justify-between mt-52 pr-16'>
                      <div className="flex items-center space-x-6">
                        {course.courseId!.duration && (
                          <div className="flex items-center text-gray-500">
                            <div className="w-10 h-10 bg-[#D4BAFC] rounded-lg flex items-center justify-center mr-3">
                              <Image src={"/assets/icons/icon.svg"} alt='duration' width={28} height={20} />
                            </div>
                            <span className="font-medium">{course.courseId!.duration} {getUIText('minutes')}</span>
                          </div>
                        )}

                        <div className="flex items-center text-gray-500">
                          <div className="w-10 h-10 bg-[#D4BAFC] rounded-lg flex items-center justify-center mr-3">
                            <Image src={"/assets/icons/wallet.svg"} alt='price' width={28} height={20} />
                          </div>
                          <span className="font-medium">{course.courseId!.price ? `$${course.courseId!.price}` : getUIText('free')}</span>
                        </div>
                      </div>
                      ...
                      {course.courseId!.thumbnail ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center">
                          <div className="text-center">
                            ...
                            <span className="text-blue-500 text-sm font-medium">{getUIText('course').toLowerCase()}</span>
                          </div>
                        </div>
                      )}
                    </div>
