    ...
    const fetchSimilarArticles = async () => {
      try {
        ...
        if (Array.isArray(article.categoryId)) {
          ...
          categoryId =
            article.categoryId.find((id) => id && id.trim() !== "") || "";
        } else if (
          article.categoryId &&
  ...
  return (
    <>
      <div className="relative w-full h-[518px]">
        ...
        />
        <div className=" absolute hidden md:flex bottom-5 right-5 gap-[10px]">
      ...
      <main className="flex justify-between flex-col md:flex-row gap-[30px] text-[#3D334A] mt-10">
        {/* Left Sidebar - Table of Contents */}
        {tableOfContents.length > 0 && (
          <div className="p-5 bg-[rgba(255,255,255,1)] md:min-h-[700px] md:h-[700px] h-auto w-full rounded-[20px] md:max-w-[335px] md:block">
        ...
        >
              ...
              <div className="flex justify-between items-center">
                <div className="flex flex-wrap gap-2">
                  {getCategoryNames(article).map((categoryName, index) => (
                    <button
                      key={index}
                      className="bg-[rgba(233,223,246,1)] rounded-[6px] p-[8px] text-[18px] uppercase leading-[90%]"
                    >
                      {categoryName}
                    </button>
                  ))}
          ...
          <div className="w-full md:pr-40 flex flex-col items-center mt-10 md:mb-20 gap-8">
            <h1 className="text-[18px] leading-[100%] tracking-[-1%] text-[#3D334A]">
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      {getCategoryNames(similarArticle)
                        .slice(0, 2)
                        .map((categoryName, index) => (
                          <span
                            key={index}
                            className="text-xs text-[rgba(132,111,160,1)] bg-[rgba(249,247,254,1)] px-2 py-1 rounded-full"
                          >
                            {categoryName}
                          </span>
                        ))}
...
const addAnchorsToContent = (content: string): string => {
  ...
  const processedContent = content
    ...
    .replace(/\[su_youtube\s+([^\]]+)\]/g, (match, attributes) => {
